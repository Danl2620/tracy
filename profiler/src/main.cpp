    ...
    if( !view )
    {
        ...
        if( ImGui::BeginPopupModal( "About Tracy", &keepOpenAbout, ImGuiWindowFlags_AlwaysAutoResize ) )
        {
            ...
            if( ImGui::TreeNode( ICON_FA_TOOLBOX " Global settings" ) )
            {
                ...
                {
                    s_config.horizontalScrollMultiplier = std::max( tmpScroll, 0.01 );
                    SaveConfig();
                }
                ...
                {
                    s_config.verticalScrollMultiplier = std::max( tmpScroll, 0.01 );
                    SaveConfig();
                }
            ...
            if( aptr != aend )
            {
                ...
                if( *ptr == ':' )
                {
                    ...
                    view = std::make_unique<tracy::View>(
                        RunOnMainThread, addrPart.c_str(), portPart, s_fixedWidth, s_smallFont, s_bigFont,
                        SetWindowTitleCallback, SetupScaleCallback, AttentionCallback, s_config, s_achievements );
                }
                else
                {
                    view = std::make_unique<tracy::View>(
                        RunOnMainThread, address.c_str(), port, s_fixedWidth, s_smallFont, s_bigFont,
                        SetWindowTitleCallback, SetupScaleCallback, AttentionCallback, s_config, s_achievements );
                }
            }
        }
